...
class MecanumRobotSender(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('serial_port', '/dev/ttyACM1')  # Default serial port
        ...
        # PWM commands for each motor
        self.pwm_commands = [-10, -10, -10, -10]

        # Publisher for STM data
        self.stm_data_pub = self.create_publisher(String, '/send_data', 10)

        # Timer for continuous sending and receiving
        self.timer = self.create_timer(0.1, self.send_and_receive_serial)
    ...
    def send_and_receive_serial(self):
        ...
        try:
            ...
            if self.serial_conn.in_waiting > 0:
                response = self.serial_conn.readline().decode(errors='replace').strip()
                self.get_logger().info(f"Received from STM: {response}")
